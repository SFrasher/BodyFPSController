...
export function registerScriptTools(server, godot) {
    ...
    server.tool("edit_script", "Edit a script using search-and-replace, full content replacement, line insertion, or 1-based inclusive line-range replacement. Restricted to .gd/.cs paths. Refuses to write a script that is currently open in Godot's script editor unless force=true is set. Content is written VERBATIM — indentation is never adjusted, so supply it matching the file. The response reports valid_after_edit, parse_errors if the edit broke the file, and indentation_warning if the new content's indent style differs from the file's.", {
    ...
    server.tool("validate_script", "Validate a GDScript file by attempting to compile it. Returns valid: true/false, plus parse_errors with the real parser message on failure. Rarely returns valid: null with indeterminate: true, when Godot skipped the compile because live instances hold the script — that means undetermined, NOT broken.", {
