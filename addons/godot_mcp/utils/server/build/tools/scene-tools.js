...
export function registerSceneTools(server, godot) {
    ...
    server.tool("create_scene", "Create a new scene file with a specified root node type. Refuses to overwrite an existing scene unless force is set.", {
        path: z.string().describe("Path for the new scene (e.g. 'res://scenes/enemy.tscn')"),
        root_type: z.string().optional().describe("Root node type (default: Node2D). Must be a concrete Node subclass. Examples: Node2D, Node3D, Control, CharacterBody2D"),
        root_name: z.string().optional().describe("Root node name (defaults to filename)"),
        force: z.boolean().optional().describe("Overwrite the scene if it already exists (default: false)"),
    }, async (params) => {
        try {
    ...
    server.tool("delete_scene", "Delete a scene file from the project. Only .tscn/.scn files are accepted, and a scene open in the editor is refused.", {
        path: z.string().describe("Path to the scene file to delete (.tscn or .scn)"),
    }, async (params) => {
        try {
